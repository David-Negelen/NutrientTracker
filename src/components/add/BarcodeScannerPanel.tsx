...
export function BarcodeScannerPanel({
  ...
  return (
    <div className="space-y-4">
      <div className="space-y-2">
        ...
        {isScannerActive ? (
          ...
          <div className="rounded-2xl border border-dashed border-slate-300 bg-slate-50 p-4 text-sm text-slate-500">
            Camera is off. Press "Use camera to scan" for the next barcode.
          </div>
        )}
      </div>
      ...
      {isLoading ? <p className="text-sm text-slate-500">Looking up product...</p> : null}
      {scannedFood ? (
        <div className="rounded-xl border border-brand-100 bg-brand-50 p-3">
          ...
          >
            <p className="font-semibold text-slate-800">{scannedFood.name}</p>
            <p className="text-sm text-slate-600">{Math.round(scannedFood.nutrients.calories)} kcal per serving</p>
          </button>
          {expandedDetails && (
          ...
          <div className="mt-3 flex gap-2">
            <button
              ...
              className={[
                ...
                addedFeedback ? "bg-emerald-600" : "bg-brand-600"
              ].join(" ")}
            >
